"""
SDE Scan for AWQ-quantized models — uses scale=0.3 instead of 0.0.
AWQ models produce NaN/inf when components are fully zeroed.

Usage:
  python exp_sde_scan_awq.py <model_path> <output_tag>
"""
...
if len(sys.argv) < 3:
    print("Usage: python exp_sde_scan_awq.py <model_path> <output_tag>")
    sys.exit(1)
...
def main():
    ...
    log(f"Using scale={SCAN_SCALE} (AWQ-safe)")
    ...
    for layer_idx in range(n_layers):
        for comp_name in ["attn", "mlp"]:
            ...
            hook = target.register_forward_hook(ScaleHook(SCAN_SCALE))
            ...
            disclaimer_rate = sum(1 for s in scores if s["has_disclaimer"]) / len(scores) if scores else 0
            ...
            status = "ERR!" if error_hit else ("CRASH!" if collapsed else ("FMT!" if fmt_changed else "OK"))
    ...
    n_errors = sum(1 for v in scan.values() if v.get("error", False))
    ...
    log(f"\nSummary: {n_crashed} crashed, {n_fmt_changed} format-changed, {n_ok} stable, {n_errors} errors / {len(scan)} total")
